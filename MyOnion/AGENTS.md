...
Follow standard C# conventions: PascalCase for types and public members, camelCase for locals/parameters, and suffix interfaces with `I`. Keep mediator requests/responses in dedicated files under `Features`, and align DTO names with their API contracts. Use expression-bodied members where they improve clarity, prefer dependency injection via constructor parameters, and run `dotnet format` (whitespace + analyzers) before reviews.
